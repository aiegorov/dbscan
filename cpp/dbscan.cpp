...
//        idx.push_back(idx_vec);
    ...
    std::cerr << "resizes took " << std::chrono::duration_cast<std::chrono::microseconds>(ts2 - ts1).count() << std::endl;
...
//    labels_outputs.reserve(x_slices_.size() - 1);
    ...
    std::cerr << "sorting took " << std::chrono::duration_cast<std::chrono::microseconds>(ts3 - ts2).count() << std::endl;
...
//        labels_outputs.push_back(fit_predict_single(points_in_slice));
    ...
    std::cerr << "execution took " << std::chrono::duration_cast<std::chrono::milliseconds>(ts4 - ts3).count() << std::endl;
